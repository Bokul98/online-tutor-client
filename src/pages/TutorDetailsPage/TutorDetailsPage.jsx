...
const TutorDetailsPage = () => {
    ...
    if (!tutor) return <div className={`text-center my-8 transition-colors duration-300 ${
    ...
    return (
        <div className={`min-h-screen flex flex-col justify-start py-10 px-2 sm:px-0 transition-all duration-300 ${
            ...
            <div className={`w-full max-w-5xl mx-auto rounded-2xl shadow-xl border flex flex-col md:flex-row items-stretch p-0 md:p-0 overflow-hidden transition-all duration-300 ${
                isDark 
                ...
                <div className="flex-shrink-0 w-full md:w-64 h-56 md:h-auto flex items-center justify-center bg-gradient-to-br from-blue-100 via-purple-100 to-pink-100">
                    <img src={tutor.image} alt={tutor.language} className="object-cover w-44 h-44 rounded-2xl shadow-lg border-4 border-white" />
                </div>
                {/* Middle: Details */}
                <div className="flex-1 w-full flex flex-col justify-center p-6 gap-4">
                    <div className="space-y-4">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <span className="text-yellow-600 font-semibold text-base">{tutor.review}</span>
                        </div>
                        <div className="flex items-center gap-3">
                        ...
                        <div className="mt-2">
                            <p className={`text-sm md:text-base leading-relaxed rounded-xl p-3 border shadow-sm transition-all duration-300 ${
                                ...
                                {showFullDesc || tutor.description.split(' ').length <= 8
                                    ? tutor.description
                                    : `${tutor.description.split(' ').slice(0, 8).join(' ')}...`}
                                {tutor.description.split(' ').length > 8 && (
                                    <button
                ...
                <div className={`flex flex-col justify-start items-end min-w-[150px] p-6 gap-4 transition-all duration-300 ${
                    ...
                    <span className="inline-block bg-gradient-to-r from-purple-500 to-pink-500 text-white text-base font-semibold px-5 py-1 rounded-full shadow mb-2">${tutor.price}</span>
                    <div className="flex-1 flex flex-col justify-end w-full">
                        <button className="px-6 py-2 bg-gradient-to-r from-green-500 to-blue-500 text-white rounded-full font-bold shadow-lg hover:scale-105 hover:bg-gradient-to-l transition-transform duration-200 text-base disabled:opacity-60 w-full md:w-auto mt-auto" onClick={handleBookNow} disabled={booking.loading}>
                            {booking.loading ? 'Booking...' : 'Book Now'}
                        </button>
                        {booking.error && <div className="text-right text-red-500 mt-2 text-sm">{booking.error}</div>}
                        {/* Remove normal success message, only show toast */}
                    </div>
                </div>
            </div>
        </div>
