      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 py-12 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10">
        ...
        <div className="lg:col-span-1">
          <Link to="/" className="flex items-center gap-2 group">
            <span className="text-2xl font-bold bg-gradient-to-r from-[#00C6FF] to-[#0072FF] bg-clip-text text-transparent group-hover:scale-105 transition-transform">
              LangLink
            </span>
          </Link>
          ...
          <div className="flex gap-3 flex-wrap">
            {socials.map((s) => (
              <a
                ...
                aria-label={s.label}
                title={s.label}
                className={`w-10 h-10 flex items-center justify-center rounded-full shadow hover:scale-110 transition-transform duration-200 ${
                  isDark ? "bg-slate-700 text-gray-100 hover:bg-slate-600" : "bg-gray-200 text-gray-800 hover:bg-gray-300"
                }`}
              >
                {s.icon}
              </a>
